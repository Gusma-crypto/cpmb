...
const props = defineProps({ routePrefix: { type: String, default: 'admin' } });
const r = (name, params) => route(`${props.routePrefix}.exam-rooms.${name}`, params);
const form = useForm({ name: '', code: '', location: '', capacity: 1, status: 'active' });
const submit = () => form.post(r('store'));
</script>
<template><Head title="Tambah Ruang Ujian" /><AdminLayout><div class="py-8"><div class="mx-auto max-w-3xl px-4"><div class="mb-6 flex justify-between"><h1 class="text-2xl font-semibold">Tambah Ruang Ujian</h1><Link :href="r('index')" class="text-sm text-blue-600">Kembali</Link></div><form class="space-y-6 bg-white p-6 shadow-sm sm:rounded-lg" @submit.prevent="submit">
